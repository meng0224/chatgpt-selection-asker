...
如果選取內容太長，腳本會暫存完整文字並開啟 ChatGPT 首頁，再嘗試自動把文字填入 ChatGPT 輸入框。這可以避免網址太長造成瀏覽器或伺服器錯誤，例如 `HTTP ERROR 431`。

暫存文字會在成功填入、過期，或 fallback 流程複製到剪貼簿後清除。

## 注意事項

- 腳本不會取代或攔截網站原本的右鍵選單，只會註冊 Tampermonkey 選單命令。
- 腳本會在 ChatGPT 頁面讀取暫存文字並嘗試填入提問欄。
- 腳本不會自動送出訊息。
- 選取內容太長時不會裁切文字。
